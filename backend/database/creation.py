...
if "login" not in collections or "users" not in collections:
    ...
    if "login" not in collections:
        for login_record in login_data:
            if db["login"].count_documents({"user_id": login_record["user_id"]}) == 0:
                login_record["password"] = hashPassword(
                    login_record["password"])
                db["login"].insert_one(login_record)
                print(f"Inserted login record for user_id: {
                      login_record['user_id']}")
            else:
                print(f"Login record for user_id: {
                      login_record['user_id']} already exists. Skipping insertion.")
    ...
    if "users" not in collections:
        for user_record in users_data:
            if db["users"].count_documents({"user_id": user_record["user_id"]}) == 0:
                ...
                print(f"Inserted user record for user_id: {
                      user_record['user_id']}")
            else:
                print(f"User record for user_id: {
                      user_record['user_id']} already exists. Skipping insertion.")
    ...
    if "movies" not in collections:
        df_movies = pd.read_csv(
            "backend/dataset/netflix_mood_recommender_test.csv", encoding='latin-1')
