...
def clean_ratings():
    ...
    netflix_dataset_df = pd.read_csv("backend/dataset/Netflix_Dataset_Movie_Filtered.csv")

    filtered_netflix_ratings = netflix_rating_df[netflix_rating_df['Movie_ID'].isin(netflix_dataset_df['Movie_ID'])]

    filtered_netflix_ratings.to_csv("backend/dataset/Netflix_Rating_Movie_Cleaned.csv", index=False)
    print("Done.")
